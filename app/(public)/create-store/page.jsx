    ...
    const fetchSellerStatus = async () => {
        ...
        try {
            ...
            if (['approved', 'rejected', 'pending'].includes(data.status)) {
                ...
                switch (data.status) {
                    case "approved":
                        setMessage("Your Store has been approved, now you can add products to your store from dashboard");
                        setTimeout(() => router.push("/store"), 5000);
                        break;
                    case "rejected":
                        setMessage("Your Store request has been rejected, please conatct the admin for more details");
                        break;
                    case "pending":
